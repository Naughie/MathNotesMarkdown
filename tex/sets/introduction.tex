...
set}）という．半順序はよく \(\le\) で表される． \((X, \le)\)
を半順序集合とするとき，2つの元 \(x, y \in X\) であって \(x \le y\) でも
\(y \le x\) でもないようなものが存在することもある．\(X\) の2元
\(x, y \in X\) は，\(x \le y\) または \(y \le x\)
であるとき\textbf{比較可能}（\emph{comparable}）といい，比較可能でないとき\textbf{比較不可能}（\emph{incomparable}）という．\(X\)
の任意の2元が比較可能であるとき，半順序 \(\le\)
